  ...
  const drawChart = useCallback((svgRef: React.RefObject<SVGSVGElement>, tooltip: MutableRefObject<null>, darkHour: boolean, updatedTiles: Tile[]) => {
    ...
    svg
      ...
      .on("click", async (event: { clientY: string; clientX: string; }, d: { column: number; row: any; }[]) => {
        ...
        if (Number(clientY) > window.innerHeight / 2) clientY = String(Number(clientY) - 350)
        if (Number(clientX) > window.innerWidth / 2) clientX = String(Number(clientX) - 350)
        tooltipElement
  ...
  return (
    <div className={styles.chart} id="chart" >
      ...
      <Card id="tooltip" ref={tooltip} className={styles.tooltip}>
        ...
        {
          tileState && tileState.landmark && <p>Landmark: {tileState.landmark}</p>
        }

        {
          tileState && tileState.hazards && tileState.hazards.length > 0 && <p>Hazards: {tileState.hazards.map((resourceName, index, resources) =>
          (
            <span key={index}>
              {resourceName} {index === resources.length - 1 ? "" : ","}&nbsp;
            </span>)
